...
For example, `starts_with()` or `contains()`. *Note: See more helper function options in my [dplyr essentials blog](../tidyverse-dplyr/index.qmd#select-rename){target="_blank"}*
...
## `matches()`
Matches a regular expression or a **stringr** [pattern](https://github.com/rstudio/cheatsheets/blob/main/strings.pdf){target="_blank"}
...
starwars %>% 
    ...
    head(3)
```
1. use of `!` as 'not'. Columns that end with 'color' and don't start with 'h'. This excludes the column 'hair_color'
...
starwars %>% 
    ...
    head(3)
```
1. select the first, third, and 4th, 5th, 6th, 7th columns
...
This is related to the data masking property that tidy evaluation employs [@programmingdplyrNDdatamasking], rather than its tidy select properties, but it still useful to know when extending tidy selection.

### With environment variables
In order to use a variable specified in the global environment (these are the variables you've assigned earlier in your script), you need to add the `!!` or `.env$` syntax around the variable to tell tidyselect to look outside of the dataframe variable [@technicaltidyselectNDenv]:
...
### In functions
You need to 'embrace' (`{{{ var }}}`) your tidyselect syntax when it is provided as an argument to a function [@programmingdplyrNDindirection]:
```{r}
